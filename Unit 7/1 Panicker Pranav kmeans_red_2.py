...
imagefile="tiger.jpg"#sys.argv[1]
K=8#int(sys.argv[2])
def naive2(val):
    if(val<128):
        return 0
...
def rep8Colors(pix):
    ...
    for x in range(w):
        for y in range(h):
            ...
            b=naive2(b)
            pix[x,y]=(r,g,b)
    return putColorsOnBottom(colorspace,pix)
def rep8ColorsDither(pix):
    a=[0,255]
    colorspace=list()
    for r in a:
        for g in a:
...
def putColorsOnBottom(colorspace,pix):
    boxsize=w/K
...
def kmeans(pix,ditherbool):
    ...
    return putColorsOnBottom(subgroups.keys(),pix)
original = Image.open(imagefile)
img=original.copy()
w,h=img.size
pix=img.load()
# output=rep8Colors(pix)
# output.show()
# output.save("tiger8NaiveNoDither.png")

# img=original.copy()
# w,h=img.size
# pix=img.load()
# output=rep8ColorsDither(pix)
# output.show()
# output.save("tiger8NaiveDither.png")

# img = original.copy()
# pix=img.load()
# output=rep27Colors(pix)
# output.show()
# output.save("tiger27NaiveNoDither.png")

# img = original.copy()
# pix=img.load()
# output=rep27ColorsDither(pix)
# output.show()
# output.save("tiger27NaiveDither.png")
...
# K=8
# output=kmeans(pix,False)
# output.save("tiger8MeansNoDither.png")
# output.show()

# img=original.copy()
# pix=img.load()
# output=kmeans(pix,True)
# output.save("tiger8MeansDither.png")
# output.show()

K=27
output=kmeans(pix,False)
output.save("tiger27MeansNoDither.png")
output.show()

img=original.copy()
pix=img.load()
output=kmeans(pix,True)
output.save("tiger27MeansDither.png")
output.show()